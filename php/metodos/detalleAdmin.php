...
class detalle extends FPDF
{
         ...
         if(((strcmp($_COOKIE["COD_CLIENTE"],"001"))!=0)){
           ...
           if ($stmt) {
            ...
            for($i=0, $size=count($row); $i<$size; $i++){
             ...
             $this->Ln();
             $this->Cell(45,5);
             $this->Cell(50,5,$row[0][9],1);
             $this->Cell(50,5,$row[0][10],1);
             $this->Ln();
  ...
  $detalle->AliasNbPages();
  //Primera página
  $detalle->AddPage();
  $detalle->SetY(65);
  $detalle->Datos($header);
  $detalle->Historial($header);
  $detalle->Output('I',$_COOKIE["COD_CLIENTE"].'.pdf');
?>
